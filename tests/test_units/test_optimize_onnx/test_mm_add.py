...
class TestMatMulAddFusion:
    ...
    @pytest.mark.parametrize(
        ("input_shape", "weight", "bias", "test_input"),
        [
            pytest.param(
                [1, 4],
    ...
    def test_fusion_numerical_correctness(self, input_shape, weight, bias, test_input):
        """MatMul+Add fusion produces numerically correct output for various shapes."""
        X = create_tensor_value_info("X", "float32", input_shape)
        inputs = [X]

        initializers = [
            create_initializer("W", weight),
            create_initializer("b", bias),
        ]

        matmul_node = helper.make_node("MatMul", inputs=["X", "W"], outputs=["matmul_output"])
        add_node = helper.make_node("Add", inputs=["matmul_output", "b"], outputs=["Y"])

        outputs = [create_tensor_value_info("Y", "float32", [input_shape[0], weight.shape[1]])]
        model = create_minimal_onnx_model([matmul_node, add_node], inputs, outputs, initializers)
