...
class TestGemmNormalization:
    ...
    @pytest.mark.parametrize(
        ("alpha", "beta"),
        [
            pytest.param(1.0, 0.5, id="absorb_beta_into_bias"),
            pytest.param(0.0, 1.0, id="alpha_zero_edge_case"),
        ],
    )
    def test_gemm_alpha_beta_combinations(self, alpha, beta):
        """Gemm with different alpha/beta combinations produces correct output."""
        X = create_tensor_value_info("X", "float32", [1, 3])
        inputs = [X]
