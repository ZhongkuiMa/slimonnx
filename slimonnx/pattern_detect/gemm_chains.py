...
def detect_gemm_gemm(
    ...
    for i in range(len(nodes) - 1):
        ...
        if not _is_consecutive_nodes(gemm1_node, gemm2_node, nodes):
            continue

        # Check if both Gemm nodes have constant weights
        if len(gemm1_node.input) < 2 or gemm1_node.input[1] not in initializers:
            continue
        if len(gemm2_node.input) < 2 or gemm2_node.input[1] not in initializers:
            continue
