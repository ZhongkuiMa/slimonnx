...
[![ONNXRuntime 1.20](https://img.shields.io/badge/ONNX%20Runtime-1.20-brightgreen.svg)](https://onnxruntime.ai)
...
- **onnx 1.17.0**: Balanced compatibility with ONNX opset 17-21 (most stable range)
- **onnxruntime 1.20.0**: Matches onnx 1.17.0 for consistent operator behavior
- **numpy 2.2.4**: Required for modern Python 3.10+ compatibility
...
```bash
python test_validation.py --preprocess-only
```

### Test Validation

```bash
python test_validation.py --validate-only
```
...
All optimizations preserve ONNXRuntime compatibility. Optimized models can be executed with onnxruntime 1.20.0.
