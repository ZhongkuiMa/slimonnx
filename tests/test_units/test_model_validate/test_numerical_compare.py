...
class TestMapInputsForModel2:
    ...
    def test_generic_input_name(self):
...
class TestCompareOutputs:
    ...
    @pytest.mark.parametrize(
        ("outputs1", "outputs2", "output_names1", "output_names2", "should_match"),
        [
            pytest.param(
            ...
            pytest.param(
                {"output": np.array([1.0, 2.0, 3.0])},
                {"output": np.array([1.0 + 1e-7, 2.0 + 1e-7, 3.0 + 1e-7])},
                ["output"],
                ["output"],
                True,
                id="within_tolerance",
            ...
            pytest.param(
                {"output": np.array([1.0, 2.0, 3.0])},
                {"output": np.array([1.0 + 1e-3, 2.0, 3.0])},
                ["output"],
                ["output"],
                False,
                id="exceeds_tolerance",
            ),
            pytest.param(
                {"out1": np.array([1.0]), "out2": np.array([2.0])},
                {"out1": np.array([1.0]), "out2": np.array([2.0])},
                ["out1", "out2"],
                ["out1", "out2"],
                True,
    ...
    def test_returns_match_status_and_mismatches(
        self, outputs1, outputs2, output_names1, output_names2, should_match
    ):
        ...
        if should_match:
            ...
            assert len(mismatches) > 0 or max_diff > 0.0
    ...
    def test_one_output_differs(self):
...
class TestGenerateTestInputs:
    ...
    def test_without_bounds(self, tmp_path):
...
class TestLoadTestInputsFromFile:
    ...
    @pytest.mark.parametrize(
        ("data", "num_samples", "expected_len"),
        [
            (np.array([1.0, 2.0, 3.0], dtype=np.float32), 5, 1),
            (
    ...
    def test_from_npy_nd(self, tmp_path, data, num_samples, expected_len):
        """Test loading arrays from .npy files."""
        ...
        if data.ndim == 1:
            ...
            for i in range(expected_len):
                assert np.array_equal(result[i]["input"], data[i])
        # [REVIEW] Merged into test_from_npy_nd via parametrize — original: test_from_npy_1d, test_from_npy_2d

    @pytest.mark.parametrize(
        "npz_key",
        [
            pytest.param("inputs", id="with_inputs_key"),
            pytest.param("X", id="with_X_key"),
        ],
    )
    def test_from_npz_with_key(self, tmp_path, npz_key):
        """Test loading from .npz file with different key names."""
        data = np.array([[1.0, 2.0], [3.0, 4.0]], dtype=np.float32)
        npz_path = tmp_path / "test.npz"
        np.savez(npz_path, **{npz_key: data})
    ...
    def test_num_samples_limit(self, tmp_path):
...
class TestRunOnnxInference:
    ...
    def test_with_explicit_input_name(self, tmp_path):
